...
export default function Mobile() {
  ...
  return (
    ...
    <div className="w-full h-full flex items-center justify-between px-5 py-3 bg-[#F5F5F5]">
      ...
      <div className='flex items-center justify-center gap-x-2 text-[#707070]'><h1 onClick={() => navigate("/AllMobile")}>See all</h1>
            <div className='flex'><LuSquareArrowLeft size={30} className=""/><LuSquareArrowRight size={30} className=""/></div></div>
    </div>
      <Slider {...settings}>
        {product_list?.map((i, index) => (
          <div id={index}  onClick={()=>navigate(`/product-detail/${i?.slug}`,{state:i})}
           className=" w-full mx-3 space-y-2 py-3 relative bg-white">
